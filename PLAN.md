...
| 3.6 | Adaptive dual-template tracking (anchor + adaptive template, update_threshold; CLI tuning flags); re-run e2e with visually-picked seeds | M | in-progress (sonnet-5) | Motivated by fable-5 review: fixed seed template drifts on real footage (plate rotation, lighting). Dual matching: max(anchor score, adaptive score); adaptive template replaced only when winning score ≥ update_threshold (~0.7) to prevent drift-by-creep. |
