...
pub fn show(ctx: &egui::Context, app: &mut TrackerApp) {
    // Task 13.7: the toolbar sits on its own strip color (design #1d1d21)
    // with a hairline bottom border, distinct from the app background.
        ...
        .show(ctx, |ui| {
            ui.horizontal(|ui| {
                ...
                if ui
                    ...
                    .on_hover_text(
                        "click a frame to mark the object to track (S toggles this mode)",
                    )
                    .clicked()
                {
